...
class IndexAction(object):
    ...
    @classmethod
    def construct(cls, idx_type, tbl, columns, col_idxs, inc_names, aux_md, raw_repr, index_counter, bias=0):
        ia = IndexAction()
    ...
    @classmethod
    def construct_md(cls, idx_name, table, idx_type, columns, inc_names, aux_md):
        ia = IndexAction()
        ia.tbl_name = table
        ia.columns = columns
        ia.idx_type = idx_type
    ...
    def aux_clause(self):
        aux_md_str = ""
        if self.aux_md is not None and len(self.aux_md) > 0:
            if "btree" in self.idx_type.lower():
                ff = 90 if int(self.aux_md[0]) == 0 else 100
                aux_md_str = f" WITH (fillfactor = {ff}) "
            elif "brin" in self.idx_type.lower():
                pp = {
                    -1: 64,
                    0: 128,
                    1: 256,
                }[int(self.aux_md[0])]
                aux_md_str = f" WITH (pages_per_range = {pp}) "
        return aux_md_str
    ...
    def sql(self, add, allow_fail=False):
        ...
        return "CREATE INDEX {flag} {idx_name} ON {tbl_name} USING {idx_type} ({columns}) {inc_clause} {aux_clause}".format(
    ...
    def __eq__(self, other):
        if type(other) is type(self):
            if self.aux_clause() != other.aux_clause():
                return False
    ...
    def __repr__(self, add=True):
        return "{a} {idx_name} ON {tbl_name} USING {idx_type} ({columns}) {inc_clause} {aux_clause}".format(
    ...
    def test_illegal_cols(self, illegal_cols):
        for icol in illegal_cols:
            if icol in self.columns or icol in self.inc_names:
                return icol
        return None
...
class IndexSpace(spaces.Tuple):
    ...
    def get_critic_dim(self):
        ...
        return index_dim + self.index_space_aux_type_dim + self.index_space_aux_include + self.index_space_aux_md
    ...
    def __init__(self,
        ...
        self.index_space_aux_type_dim = (3 if index_space_aux_brin else 2) if index_space_aux_type else 0
        self.index_space_aux_include = max_num_columns if index_space_aux_include else 0
        self.index_space_aux_md = 1 if index_space_aux_md else 0
        ...
        if index_repr == IndexRepr.ONE_HOT:
            self.index_repr_policy = OneHotIndexPolicy(self.tables, max_num_columns, index_space_aux_type, index_space_aux_brin, self.index_space_aux_include, self.index_space_aux_md, maximize=False)
            # Not allowed to use SAC for a sampling index interpretation.
        elif index_repr == IndexRepr.ONE_HOT_DETERMINISTIC:
            self.index_repr_policy = OneHotIndexPolicy(self.tables, max_num_columns, index_space_aux_type, index_space_aux_brin, self.index_space_aux_include, self.index_space_aux_md, maximize=True)
    ...
    def construct_indexaction(self, act):
        ...
        idx_type, tbl_name, col_names, col_idxs, inc_names, aux_md = self.index_repr_policy.act_to_columns(act, self.rel_metadata)
        bias = 0 if self.lsc is None else self.lsc.current_bias()
        return IndexAction.construct(idx_type, tbl_name, col_names, col_idxs, inc_names, aux_md, act, self.index_counter, bias)
    ...
    def _env_to_embedding(self, env_act):
        ...
        if self.vae is None:
            ...
            if self.index_space_aux_type_dim > 0:
                ...
                index_type = torch.zeros(index_val.shape[0], self.index_space_aux_type_dim, dtype=torch.int64)
                index_type = index_type.scatter_(1, index_val, 1).type(torch.float32)
                env_act = env_act[:, 1:]

            if self.index_space_aux_md > 0:
                md_col = torch.tensor(env_act[:, -self.index_space_aux_md:]).float()
                env_act = env_act[:, :-self.index_space_aux_md]
            ...
            if md_col is not None:
                latents = torch.concat([latents, md_col], dim=1)
    ...
    def null_action(self):
        dim = gym.spaces.utils.flatdim(self) - self.index_space_aux_type_dim - self.index_space_aux_include - self.index_space_aux_md
        action = np.zeros(dim)
        action[0] = 1
    ...
    def _sample_action_distribution(self, action, sample_num_columns=False, allow_break=True, column_override=None, omit_dominant=0): # Sample a real action given the network outputs.
        ret = self.index_repr_policy.sample_action(self.np_random, action, self.rel_metadata, sample_num_columns, allow_break=allow_break, column_override=column_override, omit_dominant=omit_dominant)
    ...
    def search_embedding_neighborhood(self, act, neighbor_parameters):
        ...
        while len(actions) == 0:
            for _ in range(neighbor_parameters["index_num_samples"]):
                sampled_action = self._sample_action_distribution(act, omit_dominant=omit_dominant)
                ...
                for candidate in candidates:
                    ...
                    if ia in self.illegal_indexes:
                        # Reject due to prior illegality confirmed.
                        if ia not in already_rejected:
                            logging.debug(f"Rejecting {ia} due to prior illegality.")
                            already_rejected.add(ia)
                        continue

                    if len(self.illegal_table_cols[ia.tbl_name]) > 0:
                        # We have some known illegal (table, col) that needs to be checked.
                        icol = ia.test_illegal_cols(self.illegal_table_cols[ia.tbl_name])
                        if icol is not None:
                            # Reject due to prior illegality confirmed.
                            if ia not in already_rejected:
                                logging.debug(f"Rejecting {ia} due to illegality on {ia.tbl_name}.{icol}")
                                already_rejected.add(ia)
                            continue
            ...
            num_enter += 1
            if num_enter >= 100:
                logging.error("Spent 100 iterations and could not find any valid index action. This should not happen. Omit dominant.")
                omit_dominant += 1

            if omit_dominant >= self.max_num_columns:
                logging.error("Now attaching the null index action.")
                actions.append(self.null_action())
                break
    ...
    def _flag_illegal(self, action, connection):
        ia = self.construct_indexaction(action)
        if ia.is_valid:
            # Only flag the illegality of valid indexes.
            self.illegal_indexes.add(ia)

            if ia.idx_type.lower() == "btree":
                # Use our understanding of btree indexes to see if there's a single column that's just illegal.
                # If so, then just reject the column as a whole.
                colnames = set(ia.columns + ia.inc_names)
                for colname in colnames:
                    try:
                        test_create = f"CREATE INDEX test_idx ON {ia.tbl_name} ({colname})"
                        connection.execute(test_create)
                        connection.execute(f"DROP INDEX test_idx")

                    except ProgramLimitExceeded as e:
                        logging.debug(f"Discovered illegal {ia.tbl_name}.{colname} with {e}")
                        self.illegal_table_cols[ia.tbl_name].add(colname)
